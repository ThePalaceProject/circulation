...
- Whenever you add, rename, remove, or change the default of an environment variable that the
  ...
  you MUST update `README.md` in the same change. This is a frequent miss — treat the doc update as
  part of the code change, not a follow-up.
- App/runtime variables are documented under the `### Environment variables` section of `README.md`,
  grouped into per-feature subsections (Database, OpenSearch, Celery, Redis, General, Storage Service,
  etc.). Add new vars to the matching subsection, or create a new subsection if none fits.
- Test-only variables (e.g. `PALACE_TEST_*`) are documented in the testing section of `README.md`
  near the other test configuration, not in `### Environment variables`.
- Follow the existing entry format for each var: name in backticks, a one-line description, whether it
  is **required** or optional, and the default value when one applies.
- A renamed or removed variable must be removed from (or updated in) `README.md` too — don't leave
