...
#### Online migrations (backwards compatibility)

Migrations are run **online**, while the current version of the application is still serving traffic. The
convention is that a release's database must work with version **N-1** of the code (the previous release).
This lets a migration run while the still-running (previous-version) webservers continue to function.

The practical consequence: a schema object (table, column, etc.) can only be dropped **one release after** the
code stops using it. Removing a schema object is therefore a two-step process across two releases:
...
`NOT NULL` with no default and depends on the new code to populate it — N-1 inserts would fail.

**Workflow — two stacked PRs.** When a change requires this two-release split, open both PRs at the same time so
the removal isn't forgotten later:

1. The **first PR** (release 1 — stop using the object, or add the column as nullable/with a server default) is
   reviewed and merged normally.
2. The **second PR** (release 2 — drop the object, or tighten the column to `NOT NULL`) is stacked on the first.
   It is reviewed but left in **draft**, and only merged after the release containing the first PR has gone out.
