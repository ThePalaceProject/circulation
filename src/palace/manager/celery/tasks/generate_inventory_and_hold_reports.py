...
# @shared_task(queue=QueueNames.high, bind=True)
# def generate_holds_report_for_titles_with_no_remaining_licenses(task: Task, library_id: int, email_address):
#    with task.session() as session:
#        generate_report_for_holds_with_no_remaining_licenses(
#            session,
#             library_id=library_id,
#             email_address=email_address,
#             send_email=task.services.email.send_email,
...
#
#
# def generate_report_for_holds_with_no_remaining_licenses(
#     session: Session,
#     library_id: int,
#     email_address: str,
#     send_email: SendEmailCallable,
...
#     if not library:
#         log.error(
#             f"Cannot generate holds report for library (id={library_id}): "
#             f"library not found."
#         )
#         return
#
#     log.info(
#         f"Starting holds report job for {library.name}({library.short_name})."
#     )
#
#     current_time = datetime.now()
#     date_str = current_time.strftime("%Y-%m-%d_%H:%M:%s")
...
#                 archive.write(
#                     filename=holds_report_file.name,
#                     arcname=f"palace-holds-with-no-liceneses-report-for-library-{file_name_modifier}.csv",
#                 )
#
#             uid = uuid_encode(uuid.uuid4())
#             key = (
#                 f"{S3Service.DOWNLOADS_PREFIX}/inventory_and_holds/{library.short_name}/"
#                 f"holds-with-no-licenses-for-library-{file_name_modifier}-{uid}.zip"
#             )
#             s3_service.store_stream(
#                 key,
#                 report_zip,
...
#             s3_file_link = s3_service.generate_url(key)
#             send_email(
#                 subject=f"Holds Reports {current_time}",
#                 receivers=[email_address],
#                 text=(
#                     f"Download Report here -> {s3_file_link} \n\n"
#                     f"This report will be available for download for 30 days."
