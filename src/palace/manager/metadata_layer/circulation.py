...
class CirculationData(BaseDataContainer):
    ...
    def _filter_and_set_defaults(self) -> Self:
        ...
        if not default_rights_uri and self.data_source is not None:
    ...
    def license_pool(
        ...
        identifier = self.primary_identifier_db(_db)

        data_source_obj = self.data_source_db(_db)
    ...
    def apply(
        ...
        data_source = self.data_source_db(_db)
        identifier = self.primary_identifier_db(_db)
