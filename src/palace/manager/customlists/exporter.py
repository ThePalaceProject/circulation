...
class CustomList:
    def __init__(self, id: int, name: str, library_id: str):
        ...
        self._problematic_books: list[ProblematicBook] = []
        self._collections: list[CollectionReference] = []
        self._id = id
        self._name = name
...
class CustomListExports:
    def __init__(self):
        self._lists: list[CustomList] = []
        self._problematic_lists: list[ProblematicCustomList] = []

    @staticmethod
    def schema() -> dict[str, Any]:
        schema_file = resources_dir("customlists") / "customlists.schema.json"
    ...
    def serialize(self) -> str:
        ...
        jsonschema.validate(document_dict, self.schema())
        return json.dumps(document_dict, sort_keys=True, indent=2)

    @classmethod
    def parse(cls, document: dict) -> Self:
        jsonschema.validate(document, cls.schema())
        exports = cls()
    ...
    @classmethod
    def parse_fd(cls, file: IO[bytes]) -> Self:
        return cls.parse(json.load(file))

    @classmethod
    def parse_file(cls, file: str) -> Self:
        with open(file, "rb") as source_file:
            return cls.parse_fd(source_file)
...
class CustomListExporter:
    ...
    def _save_customlists_document(self, document: CustomListExports) -> None:
        ...
        serialized: str = document.serialize()
