...
from palace.manager.util.filter.ast import (
...
from palace.manager.util.filter.parser import (
    DSLParser,
)
...
class DSLEvaluationVisitor(Visitor):
    ...
    # dict added so dict methods (.get(), .keys(), .values(), etc.) are callable.
    BUILTIN_CLASSES = [float, int, str, dict, types.ModuleType]
    ...
    def __init__(
        ...
        :param safe_classes: Optional list of classes which methods can be called.
            By default it contains only built-in classes: float, int, str, dict
        """
    ...
    def _get_attribute_value(obj: dict | object, attribute: str):
        ...
        if isinstance(obj, dict):
            # Key access takes precedence; fall through to getattr for dict methods
            # (.get, .keys, .values, etc.) when the name is not a key.
            if attribute in obj:
                return obj[attribute]
            if hasattr(obj, attribute):
                return getattr(obj, attribute)
            raise DSLEvaluationError(f"Cannot find attribute '{attribute}' in {obj}")
        else:
    ...
    def visit(self, node: FunctionCallExpression):
        ...
        if node.arguments:
            # Arguments are resolved in the global context, not in the scope of
            # the object the method belongs to.  Save and clear both scope
            # trackers so nested dot expressions in arguments start fresh.
            saved_scope = self._current_scope
            saved_root = self._root_dot_node
            self._current_scope = None
            self._root_dot_node = None
            ...
            self._root_dot_node = saved_root
