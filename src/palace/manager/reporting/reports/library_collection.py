...
class LibraryCollectionReport(LoggerMixin):
    ...
    def _process_table(
        ...
        :param stack: ExitStack for resource management.
        :return: TableProcessingResult with the processed data.
        """
        filename = f"{self.get_filename(key=table.definition.key)}.csv"

        # Create a temporary file opened in binary mode
        temp_file = stack.enter_context(tempfile.NamedTemporaryFile("w+b"))

        # Wrap the binary file with TextIOWrapper for text writing
        text_wrapper = io.TextIOWrapper(
            temp_file, encoding="utf-8", newline="", write_through=True
        )

        # Get the table processor bound to the text wrapper.
        processor_func = self.get_table_processor(table, file=text_wrapper)
        counting_processor = row_counter_wrapper(processor_func)

        # Process the table
        counted_rows, _ = table(counting_processor)

        # Detach the text wrapper from the binary buffer
        text_wrapper.detach()
    ...
    def _run_report(self, *, session: Session) -> bool:
        ...
        with ExitStack() as stack:
            ...
            for table in tables:
                try:
                    result = self._process_table(table, stack)
            ...
            try:
                package = self._package_results(processing_results, stack)
            except Exception as e:
