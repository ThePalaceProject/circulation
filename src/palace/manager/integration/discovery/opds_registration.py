        ...
        if (
            ...
            if session is not None:
                ...
                if MarcExporter.needs_reset_for_web_client_change(
                    ...
                    # The task is dispatched before this transaction commits,
                    # because this classmethod doesn't own the transaction. If
                    # the transaction rolls back, the reset still runs, but the
                    # cost is the same download gap as any legitimate reset (the
                    # files are regenerated by the next scheduled export run) and
                    # the settings are unchanged, so the exports are never stale.
