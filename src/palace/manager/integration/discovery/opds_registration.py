        ...
        if (
            ...
            if session is not None:
                ...
                if MarcExporter.needs_reset_for_web_client_change(
                    ...
                    # The task is dispatched before this transaction commits. If
                    # the transaction later rolls back, the reset still runs: the
                    # library's MARC files are deleted and only regenerated by
                    # the next scheduled export run. We accept that here because
                    # this classmethod doesn't own the transaction, so it can't
                    # defer dispatch until after the commit.
