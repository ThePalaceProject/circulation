...
from palace.core.coverage import BibliographicCoverageProvider
...
from palace.core.monitor import CollectionMonitor, TimelineMonitor
from palace.core.testing import DatabaseTest, MockRequestsResponse
from palace.core.util.datetime_helpers import from_timestamp, strptime_utc, utc_now
from palace.core.util.http import HTTP, BadResponseException
from palace.core.util.personal_names import sort_name_to_display_name
...
class OdiloRepresentationExtractor:
    ...
    format_data_for_odilo_format = {
        ACSM_PDF: (Representation.PDF_MEDIA_TYPE, DeliveryMechanism.ADOBE_DRM),
        ACSM_EPUB: (Representation.EPUB_MEDIA_TYPE, DeliveryMechanism.ADOBE_DRM),
        EBOOK_STREAMING: (
        ...
        "MP4": (
            Representation.MP4_MEDIA_TYPE,
            DeliveryMechanism.STREAMING_VIDEO_CONTENT_TYPE,
        ),
        "WMV": (
            Representation.WMV_MEDIA_TYPE,
            DeliveryMechanism.STREAMING_VIDEO_CONTENT_TYPE,
        ),
        "JPG": (Representation.JPEG_MEDIA_TYPE, DeliveryMechanism.NO_DRM),
        "SCORM": (Representation.SCORM_MEDIA_TYPE, DeliveryMechanism.NO_DRM),
    }
    ...
    def record_info_to_circulation(cls, availability):
        ...
        primary_identifier = IdentifierData(
            Identifier.ODILO_ID, record_id
        )  # We own this availability.
    ...
    def record_info_to_metadata(cls, book, availability):
        ...
        if published:
            try:
                ...
                cls.log.warn(
                    "Cannot parse publication date from: "
                    + published
                    + ", message: "
                    + str(e)
                )
        ...
        if last_update:
            try:
                ...
                cls.log.warn(
                    "Cannot parse last update date from: "
                    + last_update
                    + ", message: "
                    + str(e)
                )
        ...
        for subject in book.get("subjects", []):
            subjects.append(
                SubjectData(type=Subject.TAG, identifier=subject, weight=trusted_weight)
            )
        ...
        if grade_level:
            subject = SubjectData(
                type=Subject.GRADE_LEVEL, identifier=grade_level, weight=trusted_weight
            )
        ...
        if cover_image_url:
            image_data = cls.image_link_to_linkdata(
                cover_image_url, Hyperlink.THUMBNAIL_IMAGE
            )
            if image_data:
...
class OdiloAPI(BaseCirculationAPI, HasSelfTests):
    ...
    delivery_mechanism_to_internal_format = {
        v: k
        for k, v in list(
            OdiloRepresentationExtractor.format_data_for_odilo_format.items()
        )
    }
    ...
    def __init__(self, _db, collection):
        ...
        if (
            ...
            or not self.library_api_base_url
        ):
    ...
    def _run_self_tests(self, _db):
        result = self.run_test(
            "Obtaining a sitewide access token", self.check_creds, force_refresh=True
        )
        yield result
        ...
        for result in self.default_patrons(self.collection):
            ...
            yield self.run_test(task, self.get_patron_checkouts, patron, pin)
    ...
    def refresh_creds(self, credential):
        ...
        generic_error = (
            "%s may not be the right base URL. Response document was: %r"
            % (self.library_api_base_url, response.content.decode("utf-8"))
        )
        generic_exception = BadResponseException(self.TOKEN_ENDPOINT, generic_error)
    ...
    def get(self, url, extra_headers={}, exception_on_401=False):
        ...
        status_code, headers, content = self._do_get(
            self.library_api_base_url.decode("utf-8") + url, headers
        )
        if status_code == 401:
            if exception_on_401:
                # This is our second try. Give up.
    ...
    def checkout(self, patron, pin, licensepool, internal_format):
        ...
        response = self.patron_request(
            patron,
            pin,
            self.CHECKOUT_ENDPOINT.format(recordId=record_id),
        ...
        if response.content:
            ...
            if response.status_code == 404:
                self.raise_exception_on_error(
                    response_json, default_exception_class=CannotLoan
                )
            else:
                return self.loan_info_from_odilo_checkout(
                    licensepool.collection, response_json
                )
        ...
        raise CannotLoan(
            "patron: %s, record_id: %s, format: %s"
            % (patron, record_id, internal_format)
        )
    ...
    def checkin(self, patron, pin, licensepool):
        ...
        url = self.CHECKIN_ENDPOINT.format(
            checkoutId=loan["id"], patronId=patron.authorization_identifier
        )
        ...
        self.raise_exception_on_error(
            response.json(), default_exception_class=CannotReturn
        )
    ...
    def raise_exception_on_error(
        cls, data, default_exception_class=None, ignore_exception_codes=None
    ):
    ...
    def get_hold(self, patron, pin, record_id):
        ...
        for hold in patron_holds:
            if hold["recordId"] == record_id and hold["status"] in (
                "informed",
                "waiting",
            ):
                return hold
    ...
    def get_fulfillment_link(self, patron, pin, record_id, format_type):
        ...
        if (
            ...
            and loan_format
            and (
                format_type == loan_format
                or (
                    loan_format == OdiloRepresentationExtractor.ACSM
                    and format_type
                    in (
                        OdiloRepresentationExtractor.ACSM_EPUB,
                        OdiloRepresentationExtractor.ACSM_PDF,
                    )
                )
            )
        ):
            if "downloadUrl" in checkout and checkout["downloadUrl"]:
                ...
                content = None
                content_type = (
                    OdiloRepresentationExtractor.format_data_for_odilo_format[
                        format_type
                    ]
                )
        ...
        raise CannotFulfill(
            "Cannot obtain a download link for patron[%r], record_id[%s], format_type[%s].",
            patron,
            record_id,
            format_type,
        )
    ...
    def get_patron_checkouts(self, patron, pin):
        data = self.patron_request(
            patron,
            pin,
            self.PATRON_CHECKOUTS_ENDPOINT.format(
                patronId=patron.authorization_identifier
            ),
        ).json()
    ...
    def get_patron_holds(self, patron, pin):
        data = self.patron_request(
            patron,
            pin,
            self.PATRON_HOLDS_ENDPOINT.format(patronId=patron.authorization_identifier),
        ).json()
        self.raise_exception_on_error(data)
        return data
    ...
    def place_hold(self, patron, pin, licensepool, notification_email_address):
        ...
        response = self.patron_request(
            patron,
            pin,
            self.PLACE_HOLD_ENDPOINT.format(recordId=record_id),
            extra_headers={"Content-Type": "application/x-www-form-urlencoded"},
            data=payload,
        )
    ...
    def release_hold(self, patron, pin, licensepool):
        ...
        response = self.patron_request(
            patron, pin, url, extra_headers={}, data=payload, method="POST"
        )
        if response.status_code == 200:
            return True
        ...
        self.raise_exception_on_error(
            response.json(),
            default_exception_class=CannotReleaseHold,
    ...
    def get_metadata(self, record_id):
        ...
        if status_code == 200 and content:
            ...
            msg = (
                "Cannot retrieve metadata for record: "
                + record_id
                + " response http "
                + status_code
            )
    ...
    def get_availability(self, record_id):
        ...
        if status_code == 200 and len(content) > 0:
            ...
            msg = (
                "Cannot retrieve availability for record: "
                + record_id
                + " response http "
                + status_code
            )
...
class OdiloCirculationMonitor(CollectionMonitor, TimelineMonitor):
    ...
    def catch_up_from(self, start, cutoff, progress):
        ...
        self.log.info(
            "Starting recently_changed_ids, start: "
            + str(start)
            + ", cutoff: "
            + str(cutoff)
        )
        ...
        progress.achievements = "Updated records: %d. New records: %d." % (updated, new)
    ...
    def all_ids(self, modification_date=None):
        ...
        if modification_date and isinstance(modification_date, datetime.date):
            modification_date = modification_date.strftime(
                "%Y-%m-%d"
            )  # Format YYYY-MM-DD
        ...
        while status_code == 200 and len(content) > 0:
            ...
            for record in content:
                record_id = record["id"]
                self.log.info(
                    "Processing record %i/%i: %s" % (parsed, retrieved, record_id)
                )
                (
                    identifier,
                    is_new,
                ) = self.api.odilo_bibliographic_coverage_provider.process_item(
                    record_id, record
                )
        ...
        if status_code >= 400:
            self.log.error(
                "ERROR: Fail while retrieving data from remote source: HTTP "
                + status_code
            )
            if content:
            ...
            self.log.info(
                "Retrieving all ids finished ok. Retrieved %i records. New records: %i!!"
                % (retrieved, new)
            )
...
class OdiloBibliographicCoverageProvider(BibliographicCoverageProvider):
    ...
    def process_item(self, record_id, record=None):
        ...
        metadata, is_active = OdiloRepresentationExtractor.record_info_to_metadata(
            record, availability
        )
        if not metadata:
