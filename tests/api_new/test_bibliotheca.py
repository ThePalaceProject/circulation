...
@pytest.mark.skip(
    "These tests appear to be out-of-date and weren't being run due to an accidental class name collision "
    "with the class that has been renamed to TestErrorParser2."
)
class TestErrorParser:
    def test_exceeded_limit(self, bibliotheca_fixture: BibliothecaAPITestFixture):
        """The normal case--we get a helpful error message which we turn into
        an appropriate circulation exception.
        """
        msg = bibliotheca_fixture.files.sample_data("error_exceeded_limit.xml")
        error = ErrorParser().process_all(msg)
        assert isinstance(error, PatronLoanLimitReached)  # type: ignore
        assert "Patron cannot loan more than 12 documents" == error.message

    def test_exceeded_hold_limit(self, bibliotheca_fixture: BibliothecaAPITestFixture):
        msg = bibliotheca_fixture.files.sample_data("error_exceeded_hold_limit.xml")
        error = ErrorParser().process_all(msg)
        assert isinstance(error, PatronHoldLimitReached)  # type: ignore
        assert "Patron cannot have more than 15 holds" == error.message

    def test_wrong_status(self, bibliotheca_fixture: BibliothecaAPITestFixture):
        msg = bibliotheca_fixture.files.sample_data("error_no_licenses.xml")
        error = ErrorParser().process_all(msg)
        assert isinstance(error, NoLicenses)  # type: ignore
        assert (
    ...
    def test_internal_server_error_beomces_remote_initiated_server_error(
        ...
        error = ErrorParser().process_all(msg)
        assert isinstance(error, RemoteInitiatedServerError)  # type: ignore
    ...
    def test_unknown_error_becomes_remote_initiated_server_error(
        self, bibliotheca_fixture: BibliothecaAPITestFixture
    ):
        """Simulate the message we get when the server gives a vague error."""
        msg = bibliotheca_fixture.files.sample_data("error_unknown.xml")
        error = ErrorParser().process_all(msg)
        assert isinstance(error, RemoteInitiatedServerError)  # type: ignore
        assert BibliothecaAPI.SERVICE_NAME == error.service_name
    ...
    def test_remote_authentication_failed_becomes_remote_initiated_server_error(
        ...
        set up correctly.
        """
        msg = bibliotheca_fixture.files.sample_data("error_authentication_failed.xml")
        error = ErrorParser().process_all(msg)
        assert isinstance(error, RemoteInitiatedServerError)  # type: ignore
        assert BibliothecaAPI.SERVICE_NAME == error.service_name
    ...
    def test_blank_error_message_becomes_remote_initiated_server_error(
        self, bibliotheca_fixture: BibliothecaAPITestFixture
    ):
