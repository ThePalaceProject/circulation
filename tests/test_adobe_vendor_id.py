...
class TestVendorIDModel(VendorIDTest):
    ...
    def setup_method(self):
        ...
        # libraries.
        self.initialize_adobe(self.vendor_id_library, [self.short_client_token_library])
        ...
        self.model = AdobeVendorIDModel(
            self._db, self._default_library, self.authenticator, self.TEST_NODE_VALUE
        )
    ...
    def test_uuid_and_label_creates_delegatedpatronid_from_credential(self):
        ...
        old_style_credential = Credential.lookup(
            self._db,
            adobe,
            self.model.VENDOR_ID_UUID_TOKEN_TYPE,
            self.bob_patron,
            set_value,
            True,
        )
        ...
        [bob_delegated_patron_identifier] = (
            self._db.query(DelegatedPatronIdentifier)
            .filter(
    ...
    def test_create_authdata(self):
        ...
        bob_authdata = Credential.lookup(
            self._db,
            self.data_source,
            self.model.AUTHDATA_TOKEN_TYPE,
            self.bob_patron,
            None,
        )
    ...
    def test_to_delegated_patron_identifier_uuid(self):
        ...
        assert (None, None) == self.model.to_delegated_patron_identifier_uuid(
            foreign_uri, None
        )
        assert (None, None) == self.model.to_delegated_patron_identifier_uuid(
    ...
    def test_short_client_token_lookup_delegated_patron_identifier_success(self):
        ...
        uuid, label = self.model.short_client_token_lookup(token, signature)
    ...
    def test_short_client_token_lookup_delegated_patron_identifier_failure(self):
        uuid, label = self.model.short_client_token_lookup("bad token", "bad signature")
        assert None == uuid
        assert None == label
    ...
    def test_authdata_token_credential_lookup_success(self):
        ...
        token, ignore = Credential.persistent_token_create(
            self._db, self.data_source, self.model.AUTHDATA_TOKEN_TYPE, self.bob_patron
        )
...
class TestAuthdataUtility(VendorIDTest):
    ...
    def test_from_config(self):
        ...
        self.adobe_vendor_id.set_setting(AuthdataUtility.OTHER_LIBRARIES_KEY, None)
    ...
    def test_cannot_decode_token_from_future(self):
        ...
        authdata = self.authdata._encode("Patron identifier", iat=future)
        pytest.raises(InvalidIssuedAtError, self.authdata.decode, authdata)

    def test_cannot_decode_expired_token(self):
        expires = datetime_utc(2016, 1, 1, 12, 0, 0)
        authdata = self.authdata._encode("Patron identifier", exp=expires)
        pytest.raises(ExpiredSignatureError, self.authdata.decode, authdata)
    ...
    def test_migrate_adobe_id_success(self):
        ...
        new_credential_2, delegated_identifier_2 = self.authdata.migrate_adobe_id(
            patron
        )
