...
class TestImportCollection:
    ...
    def test_import_collection_with_import_all(
        ...
        assert (
            import_call.kwargs["modified_since"] == OverdriveImporter.DEFAULT_START_TIME
        )
    ...
    def test_import_collection_identifier_set_tracking(
        ...
        # Verify result is the identifier set
    ...
    @patch("palace.manager.celery.tasks.overdrive.OverdriveImporter")
    def test_import_collection_pagination_limit(
        self,
        mock_importer_class: MagicMock,
        overdrive_import_fixture: OverdriveImportFixture,
        db: DatabaseTransactionFixture,
    ):
        """Test that import_collection stops replacing after processing 101+ items."""
        collection = overdrive_import_fixture.collection
        ...
        # Mock result with next page and 102 items processed
        next_endpoint = BookInfoEndpoint(url="http://test.com/page2")
        mock_result = FeedImportResult(
            current_page=BookInfoEndpoint(url="http://test.com/page1"),
            next_page=next_endpoint,
            processed_count=102,
        )
        mock_importer.import_collection.return_value = mock_result
        ...
        # Run the task - should NOT replace itself despite having next_page
        result = overdrive.import_collection.delay(collection.id).wait()

        # Should return the identifier set instead of replacing
        assert isinstance(result, IdentifierSet)
...
class TestRehydrateIdentifierSet:
    ...
    def test_rehydrate_identifier_set(
        ...
        assert isinstance(result, IdentifierSet)
        assert result.key == ["test", "key", "parts"]
...
class TestRemoveIdentifierSet:
    ...
    def test_remove_identifier_set_success(
        ...
        mock_id = Mock()
        mock_id.identifier = "test-id"
        identifier_set.add(mock_id)
...
class TestIntegration:
    ...
    def test_full_import_workflow_single_page(
        ...
        # Verify result is identifier set
        assert isinstance(result, IdentifierSet)
    ...
    def test_full_import_workflow_with_parent_identifiers(
        ...
        mock_id = Mock()
        mock_id.identifier = "parent-id-1"
        parent_set.add(mock_id)

        parent_identifiers = {"key": parent_set.key}
        ...
        overdrive.import_collection.delay(
            collection.id, parent_identifiers=parent_identifiers
        ).wait()
        ...
        assert call_kwargs["parent_identifier_set"].key == parent_set.key
