...
class TestLibraryAllTitleTable:
    ...
    def test_definition(self, db: DatabaseTransactionFixture):
        ...
        table = LibraryAllTitleReportTable(library=library)
...
class TestLibraryAllTitleReportTable:
    ...
    def test_included_collections(self, db: DatabaseTransactionFixture):
        ...
        # since it is immutable. So, we patch the statement and the session
        # execute method to try to get a view into what is going on.
        ...
        assert LibraryAllTitleReportTable.included_collections(library) == [
            active_collection
        ]

        table = LibraryAllTitleReportTable(library=library)
        assert table.included_collections(library) == [active_collection]
        ...
        table = LibraryAllTitleReportTable(
            library=library, collection_ids=test_collection_ids
        )
        ...
        assert set(
            table.included_collections(library, collection_ids=test_collection_ids)
        ) == {active_collection, inactive_collection}
