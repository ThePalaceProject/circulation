...
@pytest.fixture
def marc_file_uploads_fixture(redis_fixture: RedisFixture):
    return MarcFileUploadsFixture(redis_fixture)


class TestMarcFileUploads:
    def test__pipeline(self, marc_file_uploads_fixture: MarcFileUploadsFixture):
        uploads = marc_file_uploads_fixture.uploads

        # Using the _pipeline() context manager makes sure that we hold the lock
        with pytest.raises(RedisMarcError) as exc_info:
    ...
    def test__execute_pipeline(
        self,
        marc_file_uploads_fixture: MarcFileUploadsFixture,
    ...
    def test_append_buffers(self, marc_file_uploads_fixture: MarcFileUploadsFixture):
        uploads = marc_file_uploads_fixture.uploads

        # If we try to update buffers without acquiring the lock, we should get an error
        with pytest.raises(RedisMarcError) as exc_info:
            uploads.append_buffers(
                {marc_file_uploads_fixture.mock_upload_key_1: "test"}
            )
        assert "Must hold lock" in str(exc_info.value)
        ...
        with uploads.lock() as locked:
            ...
            assert uploads.append_buffers(
                {
                    marc_file_uploads_fixture.mock_upload_key_1: "test",
                    marc_file_uploads_fixture.mock_upload_key_2: "another_test",
                }
            ) == {
                marc_file_uploads_fixture.mock_upload_key_1: 4,
                marc_file_uploads_fixture.mock_upload_key_2: 12,
            }
            assert uploads._update_number == 2

            assert uploads.append_buffers(
                {
                    marc_file_uploads_fixture.mock_upload_key_1: "x",
                    marc_file_uploads_fixture.mock_upload_key_2: "y",
                    marc_file_uploads_fixture.mock_upload_key_3: "new",
                }
            ) == {
                marc_file_uploads_fixture.mock_upload_key_1: 5,
                marc_file_uploads_fixture.mock_upload_key_2: 13,
                marc_file_uploads_fixture.mock_upload_key_3: 3,
            }
            ...
            with pytest.raises(RedisMarcError) as exc_info:
                uploads.append_buffers(marc_file_uploads_fixture.test_data)
            assert "Update number mismatch" in str(exc_info.value)

        # Exiting the context manager should release the lock
        assert not uploads.locked()

    def test_get(self, marc_file_uploads_fixture: MarcFileUploadsFixture):
        uploads = marc_file_uploads_fixture.uploads

        assert uploads.get() == {}
        assert uploads.get(marc_file_uploads_fixture.mock_upload_key_1) == {}

        marc_file_uploads_fixture.load_test_data()
        ...
        assert uploads.get(
            marc_file_uploads_fixture.mock_upload_key_1,
        ) == marc_file_uploads_fixture.test_data_records(
            marc_file_uploads_fixture.mock_upload_key_1
        )
        ...
        assert uploads.get(
            [
                marc_file_uploads_fixture.mock_upload_key_1,
                marc_file_uploads_fixture.mock_upload_key_2,
                marc_file_uploads_fixture.mock_unset_upload_key,
            ]
        ) == marc_file_uploads_fixture.test_data_records(
            marc_file_uploads_fixture.mock_upload_key_1,
            marc_file_uploads_fixture.mock_upload_key_2,
        ...
        assert uploads.get() == marc_file_uploads_fixture.test_data_records(
            marc_file_uploads_fixture.mock_upload_key_1,
            marc_file_uploads_fixture.mock_upload_key_2,
            marc_file_uploads_fixture.mock_upload_key_3,
        )
    ...
    def test_set_upload_id(self, marc_file_uploads_fixture: MarcFileUploadsFixture):
        ...
        with pytest.raises(RedisMarcError) as exc_info:
            uploads.set_upload_id(marc_file_uploads_fixture.mock_upload_key_1, "xyz")
        assert "Must hold lock" in str(exc_info.value)
        ...
        marc_file_uploads_fixture.load_test_data()
        uploads.set_upload_id(marc_file_uploads_fixture.mock_upload_key_1, "def")
        uploads.set_upload_id(marc_file_uploads_fixture.mock_upload_key_2, "abc")

        all_uploads = uploads.get()
        assert (
            all_uploads[marc_file_uploads_fixture.mock_upload_key_1].upload_id == "def"
        )
        assert (
            all_uploads[marc_file_uploads_fixture.mock_upload_key_2].upload_id == "abc"
        )

        # We can't change the upload id for a library that has already been set
        with pytest.raises(RedisMarcError) as exc_info:
            uploads.set_upload_id(marc_file_uploads_fixture.mock_upload_key_1, "ghi")
        assert "Failed to set upload ID" in str(exc_info.value)
    ...
    def test_clear_uploads(self, marc_file_uploads_fixture: MarcFileUploadsFixture):
        uploads = marc_file_uploads_fixture.uploads
    ...
    def test_get_upload_ids(self, marc_file_uploads_fixture: MarcFileUploadsFixture):
        uploads = marc_file_uploads_fixture.uploads

        # If the id is not set, we should get None
        assert uploads.get_upload_ids(
            [marc_file_uploads_fixture.mock_upload_key_1]
        ) == {marc_file_uploads_fixture.mock_upload_key_1: None}
        ...
        with uploads.lock() as locked:
            assert locked
            uploads.set_upload_id(marc_file_uploads_fixture.mock_upload_key_1, "abc")
            uploads.set_upload_id(marc_file_uploads_fixture.mock_upload_key_2, "def")
        assert uploads.get_upload_ids(marc_file_uploads_fixture.mock_upload_key_1) == {
            marc_file_uploads_fixture.mock_upload_key_1: "abc"
        }
        assert uploads.get_upload_ids(
            [
                marc_file_uploads_fixture.mock_upload_key_1,
            ...
            marc_file_uploads_fixture.mock_upload_key_1: "abc",
            marc_file_uploads_fixture.mock_upload_key_2: "def",
        }
    ...
    def test_add_part_and_clear_buffer(
        ...
        with pytest.raises(RedisMarcError) as exc_info:
            uploads.add_part_and_clear_buffer(
                marc_file_uploads_fixture.mock_upload_key_1,
                marc_file_uploads_fixture.part_1,
            )
        assert "Must hold lock" in str(exc_info.value)
        ...
        uploads.add_part_and_clear_buffer(
            marc_file_uploads_fixture.mock_upload_key_1,
            marc_file_uploads_fixture.part_2,
        )

        all_uploads = uploads.get()
        # The parts are added in order and the buffers are cleared
        assert all_uploads[marc_file_uploads_fixture.mock_upload_key_1].parts == [
            marc_file_uploads_fixture.part_1,
            marc_file_uploads_fixture.part_2,
        ]
        assert all_uploads[marc_file_uploads_fixture.mock_upload_key_2].parts == [
            marc_file_uploads_fixture.part_1
        ]
        assert all_uploads[marc_file_uploads_fixture.mock_upload_key_1].buffer == ""
        assert all_uploads[marc_file_uploads_fixture.mock_upload_key_2].buffer == ""
    ...
    def test_get_part_num_and_buffer(
        ...
        with pytest.raises(RedisMarcError) as exc_info:
            uploads.get_part_num_and_buffer(marc_file_uploads_fixture.mock_upload_key_1)
        ...
        assert uploads.get_part_num_and_buffer(
            ...
            0,
            marc_file_uploads_fixture.test_data[
                marc_file_uploads_fixture.mock_upload_key_1
            ],
        )
        ...
        with uploads.lock() as locked:
            ...
            uploads.append_buffers(
                {
                    marc_file_uploads_fixture.mock_upload_key_1: "1234567",
                }
            )
