...
def test_boost() -> None:
    ...
    boosted_multiple = boost(4.5, [q1, q2])
    ...
    boosted_multiple = boost(4.5, [q1, q2], all_must_match=True)
    assert "bool" == boosted_multiple.name
    assert 4.5 == boosted_multiple.boost
...
def test_nest() -> None:
    # Test the nest function, which turns a normal query into a
    # nested query.
    query = Term(**{"nested_field": "value"})
    nested = nest("subdocument", query)
    assert Nested(path="subdocument", query=query) == nested


def test_nestable() -> None:
    # Test the nestable helper function, which turns a normal
    ...
    assert normal_query == nestable(field, normal_query)
    ...
    nested = nestable(field, subdocument_query)
    assert Nested(path="contributors", query=subdocument_query) == nested


def test_match_term() -> None:
    # match_term creates a Match Opensearch object which does a
    # match against a specific field.
    qu = match_term("author", "flannery o'connor")
    ...
    field = "genres.name"
    qu = match_term(field, "Biography")
    assert Nested(path="genres", query=Term(**{field: "Biography"})) == qu


def test_match_range() -> None:
    # Test the match_range helper function.
...
def test_make_target_age_query() -> None:
    ...
    as_filter, as_query = make_target_age_query((5, 10))
