...
def test_task_session_maker() -> None:
    ...
    with (
        patch("core.celery.task.SessionManager") as mock_session_manager,
        patch("core.celery.task.sessionmaker") as mock_sessionmaker,
    ):
...
def test_task_services() -> None:
    ...
    with patch("core.celery.task.container_instance") as mock_container_instance:
