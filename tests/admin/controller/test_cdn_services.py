...
class TestCDNServices(SettingsControllerTest):
    ...
    def test_cdn_services_get_with_one_service(self):
        ...
        with self.request_context_with_admin("/"):
            ...
            assert "mirrored domain" == service.get("settings").get(
                Configuration.CDN_MIRRORED_DOMAIN_KEY
            )
    ...
    def test_cdn_services_post_create(self):
        ...
        service = get_one(
            self._db, ExternalIntegration, goal=ExternalIntegration.CDN_GOAL
        )
        ...
        assert (
            "mirrored domain"
            == service.setting(Configuration.CDN_MIRRORED_DOMAIN_KEY).value
        )
    ...
    def test_cdn_services_post_edit(self):
        ...
        assert (
            "new mirrored domain"
            == cdn_service.setting(Configuration.CDN_MIRRORED_DOMAIN_KEY).value
        )

    def test_check_name_unique(self):
        kwargs = dict(
            protocol=ExternalIntegration.CDN, goal=ExternalIntegration.CDN_GOAL
        )
    ...
    def test_cdn_service_delete(self):
        ...
        with self.request_context_with_admin("/", method="DELETE"):
            ...
            response = self.manager.admin_cdn_services_controller.process_delete(
                cdn_service.id
            )
            assert response.status_code == 200
