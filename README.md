...
```sh
docker run -d --name es -e discovery.type=single-node -p 9200:9200 cm_local_es_arm64
...
The admin interface is at `http://localhost:6500/admin/`. See [Usage](#Usage) for an overview of the admin interface.

### Alternative: Download Elasticsearch and Postgres Locally
